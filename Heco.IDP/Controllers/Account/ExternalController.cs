...
namespace Heco.IDP.Controllers.Account
{
    ...
    public class ExternalController : Controller
    {
        ...
        public async Task<IActionResult> Callback()
        {
            ...
            if (user == null)
            {
                ...
                if (provider == "Facebook")
                {
                    ...
                    if (email != null)
                    {
                        var userByEmail = _hecoUserRepository.GetUserByEmail(email.Value);
                        if (userByEmail != null)
                        {
                            // add Facebook as a provider for this user
                            _hecoUserRepository.AddUserLogin(userByEmail.Id, provider, providerUserId);
        ...
        private (ApplicationUser user, string provider, string providerUserId, IEnumerable<Claim> claims) FindUserFromExternalProvider(AuthenticateResult result)
        {
            ...
            // find external user
            var user = _hecoUserRepository.GetUserByProvider(provider, providerUserId);
        ...
        //{
        //    var user = _hecoUserRepository.AutoProvisionUser(provider, providerUserId, claims.ToList());
        //    return user;
        //}
